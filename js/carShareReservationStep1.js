...
function verif(){
    ...
    if (depMinute>0 && depMinute<15) {depMinute="15";}
    if(depMinute>15 && depMinute<30){depMinute="30";}
    if (depMinute>30 && depMinute<45) {depMinute="45";}
    ...
    if (depMinute==0) {depMinute="00";}
    if(depHour<10) {depHour = "0" + depHour;}
    ...
    if (endMinute>0 && endMinute<15) {endMinute="15";}
    if (endMinute>15 && endMinute<30){endMinute="30";}
    if (endMinute>30 && endMinute<45) {endMinute="45";}
    ...
    if (endHour<10) {endHour = "0" + depHour;}
    if (depMonthRet < 10) {depMonthRet = "0"+depMonthRet;}
    if (depDayRet < 10) {depDayRet = "0"+depDayRet;}

    var endTime = (endHour+':'+endMinute);
    var retFinal = depMonthRet+"/"+depDayRet+"/"+returningForm.getFullYear()+" "+endTime;
    var departureDate = new Date(depFinal);
    var returningDate = new Date(retFinal);

    if (departureDate>returningDate){
        document.getElementById("errorMessage").textContent="Please verify the dates";
    }
    else {
        window.top.location="carShareReservationStep2.html?departure="+depFinal+"&returning="+retFinal;
    }
}
